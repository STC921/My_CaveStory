...
namespace{
	...
	//const units::Tile kHealthFillSourceWidth = 5 * units::kHalfTile;
...
Player::Health::Health(Graphics& graphics, units::HP setCurrent) :
	damage_(0),
	damage_timer_(kDamageDelay),
	current_health_(setCurrent),
	max_health_(6),
	health_bar_sprite_(
		graphics,
		kSpritePath,
...
void Player::Health::update(units::MS elapsed_time_ms){
	if (damage_ && damage_timer_.expired()){
...
units::HP Player::Health::getCurrentHealth(){
	return current_health_;
}

void Player::Health::setCurrentHealth(units::HP health){

	...
	ENUM_FOREACH(h_facing, HORIZONTAL_FACING){
		ENUM_FOREACH(v_facing, VERTICAL_FACING){
			initializeSprite(graphics,
				boost::make_tuple((HorizontalFacing)h_facing,
...
void PolarStar::initializeSprite(Graphics& graphics, const SpriteState& sprite_state){
	...
	sprite_map_[sprite_state] = boost::shared_ptr<Sprite>(new Sprite(
		...
		units::gameToPixel(kPolarSatrIndex * kGunWidth), units::tileToPixel(tile_y),
...
void PolarStar::draw(Graphics& graphics,
	...
	//const units::Game bullet_x = x + kNozzleHorizontalLeftX - units::kHalfTile;
	//const units::Game bullet_y = y + kNozzleHorizontalY - units::kHalfTile;
}
...
PolarStar::Projectile::Projectile(boost::shared_ptr<Sprite> sprite,
	...
	units::Game x, units::Game y) : 
		...
		x_(x), y_(y), offset_(0), alive_(true){}

Rectangle PolarStar::Projectile::collisionRectangle() const{
	units::Game width = vertical_direction_ == HORIZONTAL ? units::tileToGame(1) : kProjectileWidth;
	units::Game height = vertical_direction_ != HORIZONTAL ? units::tileToGame(1) : kProjectileWidth;
	return Rectangle(getX() + units::kHalfTile - width / 2, getY() + units::kHalfTile - height / 2, width, height);
}
...
void PolarStar::startFire(units::Game player_x, units::Game player_y, 
	...
	if (!projectile_a_){
		...
		projectile_a_.reset(new Projectile(
			...
			horizontal_facing, vertical_facing, bullet_x, bullet_y));
	}
